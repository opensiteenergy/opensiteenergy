...
sudo -u postgres psql -d ${POSTGRES_DB} -c "CREATE EXTENSION postgis;" | tee -a /usr/src/opensiteenergy/opensiteenergy.log
sudo -u postgres psql -d ${POSTGRES_DB} -c "CREATE EXTENSION postgis_raster;" | tee -a /usr/src/opensiteenergy/opensiteenergy.log
sudo -u postgres psql -d ${POSTGRES_DB} -c "GRANT ALL PRIVILEGES ON DATABASE ${POSTGRES_DB} TO ${POSTGRES_USER};" | tee -a /usr/src/opensiteenergy/opensiteenergy.log
